...
function start_redis(){
    ...
    if [[ $? -ne 0 ]];then
    load "emic_redis.${CONFIG_VERSION_REDIS}.tar"
    fi
  ...
  #docker在run后 倒数第2步执行ENTRYPOINT。倒数第一步执行cmd，cmd也可以不执行，如果docker run结束以后加了 执行程序 例如 /bin/bash
    ...
    command="docker run -it -d  --restart="${CONFIG_RESTART_MODE}" --network=${CONFIG_NETWORK_MODE}  --name emi_redis"
    ...
    command="$command emic_redis:${CONFIG_VERSION_REDIS} "
...
function start_mysql(){
 ...
 #data
    log_info "check [CONFIG_MYSQL_LIB_PATH] !"
    if [[ -d ${CONFIG_MYSQL_LIB_PATH} ]];then
      log_info "[CONFIG_MYSQL_LIB_PATH] already existed"

#已经安装过了
      if [[ -d ${CONFIG_MYSQL_LIB_PATH}/mysql ]];then
          log_info "[${CONFIG_MYSQL_LIB_PATH}/mysql] already existed"
          mv ${CONFIG_MYSQL_LIB_PATH}/mysql ${CONFIG_MYSQL_LIB_PATH}/mysql_".bak"$(date "+%Y%m%H")
          #exit  1
      fi
    fi
    mkdir ${CONFIG_MYSQL_LIB_PATH} -p
#数据准备
    tar -zxvf ${CONFIG_MYSQL_RESOURCE_PATH}/mysql_${DATA_VERSION}.tar.gz -C ${MYSQL_LIB_PATH}
#
    log_info "Starting container for mysql！"
    command="docker run -d --name emic_mysql --restart=unless-stopped --network=host"
    command="${command} -v ${CONFIG_MYSQL_CONF_PATH}:/etc/mysql/conf.d"
    command="${command} -v ${CONFIG_MYSQL_LIB_PATH}/mysql:/var/lib/mysql"
...
#配置
    ...
    conf_m="${CONFIG_MYSQL_CONF_PATH}/my.cnf"
    if [[ -f ${conf_m} ]];then
      mv $conf_m ${conf_m}".bak"$(date "+%Y%m%d")
     fi
     docker restart emic_mysql
    if [[ $? -ne 0 ]];then
      echo "重启 mysql 容器失败"
      exit 1
    fi
    new_container=`docker ps | grep "emic_mysql" | wc -l`
    if [[ "$new_container" -gt "0" ]];then
  ...
  #设置数据库用户名和属性
  #myCommand="docker exec -it emic_mysql mysql -uroot -p'Sinicnet@123456' -h 192.168.1.200  -P 13306"
