...
function get_local_ip(){
  ...
  LOCAL_IP=`ifconfig "$networkCard"|grep inet|grep -v inet6|awk '{print $2}'`
  return $LOCAL_IP
}
cd $BASEHOME
#读取配置
...
CONFIG_OUTCALL_SERVER_HOST=`cat $CONFIGFILE | grep "^outcall_server_port" | sed "s/outcall_server_port=//g" | tr -d "\r\n"`
...
#V3.0 add
CONFIG_SERVER_LIST=`cat $CONFIGFILE | grep "^server_list" | sed "s/server_list=//g" | tr -d "\r\n"`
...
#数据库 （docker版本的）
CONFIG_MYSQL_VERSION=`cat $CONFIGFILE | grep "^version_mysql" | sed "s/version_mysql=//g" | tr -d "\r\n\[a-z]"`   #5.7.35
CONFIG_MYSQL_DATA_VERSION=`cat $CONFIGFILE | grep "^mysql_data_version" | sed "s/mysql_data_version=//g" | tr -d "\r\n\[a-z]"` #v41.5.82550
CONFIG_MYSQL_LIB_PATH=`cat $CONFIGFILE | grep "^mysql_lib_path" | sed "s/mysql_lib_path=//g" | tr -d "\r\n"`    #/var/pbx/lib/data/mysql_${mysql_data_version}
CONFIG_MYSQL_CONF_PATH=`cat $CONFIGFILE | grep "^mysql_config_path" | sed "s/mysql_config_path=//g" | tr -d "\r\n"`  #/etc/pbx/mysql_emic
CONFIG_MYSQL_RESOURCE_PATH=$BASEHOME/src/base/mysql
...
#analysis:elasticsearch 镜像 ,spring 服务镜像
CONFIG_VERSION_ELASTICSEARCH=`cat $CONFIGFILE | grep "^version_analysis-elasticsearch" | sed "s/version_analysis-elasticsearch=//g" | tr -d "\r\n"`
CONFIF_VERSION_SPRING=`cat $CONFIGFILE | grep "^version_ai-analysis" | sed "s/version_ai-analysis=//g" | tr -d "\r\n"`
#asr tts数量
CONFIG_NUM_TTS=`cat $CONFIGFILE | grep "^tts_num" | sed "s/tts_num=//g" | tr -d "\r\n"`
